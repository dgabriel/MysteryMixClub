...
the Playwright REPL at `.Codex/skills/run-mysterymixclub/driver.mjs` — this
...
```bash
(cd .Codex/skills/run-mysterymixclub && npm install)   # installs playwright
npx --prefix .Codex/skills/run-mysterymixclub playwright install chromium
```
...
```bash
node .Codex/skills/run-mysterymixclub/driver.mjs <<'EOF'
launch
nav /
...
tmux send-keys -t mmc 'node .Codex/skills/run-mysterymixclub/driver.mjs' Enter
...
- **`npx playwright install chromium` warns about missing project deps
  ...
  `.Codex/skills/run-mysterymixclub/` first (see Prerequisites), then
