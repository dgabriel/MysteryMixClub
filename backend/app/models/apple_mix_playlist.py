...
class AppleMixPlaylist(Base):
    """One Apple Music library playlist per (mix, user) — MYS-108.
    ...
    mix page can surface a link on later visits.
    """

    # Renamed from AppleRoundPlaylist (MYS-195, R3/R4 identifier cleanup) — the
    # DB was already renamed to club/mix vocabulary in MYS-196, so the attribute
    # names now match the column names directly with no seam.
    ...
    # Set when a rollback reopens the mix (MYS-168): the playlist in the
    # member's library no longer reflects the mix's submissions. The row is
