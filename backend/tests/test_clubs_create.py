"""Tests for MYS-12: POST /api/v1/clubs (create club).
...
Covers auth (401), happy-path response shape, persistence of the clubs row
and the organizer's club_members row, the votes_per_player default, the
...
async def test_unauthenticated_create_returns_401(client):
    resp = await client.post(CLUBS_URL, json=_valid_body())
...
async def test_create_returns_201_and_full_club_shape(client, db_session):
    ...
    resp = await client.post(CLUBS_URL, headers=_auth_header(user.id), json=body)
    ...
    assert data["description"] == "A club for hot tracks"
...
async def test_create_persists_club_and_organizer_membership(client, db_session):
    user = await _seed_user(db_session)
    user_id = user.id

    resp = await client.post(CLUBS_URL, headers=_auth_header(user_id), json=_valid_body())

    assert resp.status_code == 201, resp.text

    db_session.expire_all()

    clubs = (await db_session.scalars(select(Club))).all()
    assert len(clubs) == 1
    club = clubs[0]
    assert club.organizer_id == user_id
...
async def test_songs_per_submission_accepts_up_to_5(client, db_session):
    ...
    resp = await client.post(
        CLUBS_URL, headers=_auth_header(user.id), json=_valid_body(songs_per_submission=5)
    )
...
async def test_songs_per_submission_below_1_returns_422(client, db_session):
    ...
    resp = await client.post(
        CLUBS_URL, headers=_auth_header(user.id), json=_valid_body(songs_per_submission=0)
    )
...
async def test_songs_per_submission_above_5_returns_422(client, db_session):
    ...
    resp = await client.post(
        CLUBS_URL, headers=_auth_header(user.id), json=_valid_body(songs_per_submission=6)
    )
...
async def test_empty_name_returns_422(client, db_session):
    ...
    resp = await client.post(CLUBS_URL, headers=_auth_header(user.id), json=_valid_body(name=""))
...
async def test_name_too_long_returns_422(client, db_session):
    ...
    resp = await client.post(
        CLUBS_URL, headers=_auth_header(user.id), json=_valid_body(name="x" * 101)
    )

    assert resp.status_code == 422, resp.text


async def test_missing_total_mixes_defaults_to_6(client, db_session):
    # total_mixes now defaults to 6 (MYS-62): omitting it is valid and yields a
    # 201 with six auto-generated pending mixes.
    user = await _seed_user(db_session)
    body = _valid_body()
...
async def test_total_mixes_below_one_returns_422(client, db_session):
    ...
    resp = await client.post(
        CLUBS_URL, headers=_auth_header(user.id), json=_valid_body(**{"total_mixes": 0})
    )
