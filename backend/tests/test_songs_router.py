...
The Odesli and Spotify service dependencies are overridden with in-memory fakes
...
class _FakeSpotify:
    def __init__(
        self, *, result: SpotifySearchResult | None = None, error: Exception | None = None
    ):
        self._result = result
        self._error = error
...
def _build_client(session_factory, *, odesli=None, spotify=None) -> AsyncClient:
    app = create_app()
...
async def test_search_requires_auth(session_factory):
    async with _build_client(
        session_factory, spotify=_FakeSpotify(result=SpotifySearchResult(results=[]))
    ) as client:
...
async def test_search_happy_path(session_factory, db_session):
    ...
    async with _build_client(session_factory, spotify=_FakeSpotify(result=result)) as client:
...
async def test_search_surfaces_too_many_results(session_factory, db_session):
    user = await _seed_user(db_session)
    result = SpotifySearchResult(results=[], too_many_results=True)
    async with _build_client(session_factory, spotify=_FakeSpotify(result=result)) as client:
...
@pytest.mark.parametrize(
    ("error", "expected"),
    [
        (SpotifyRateLimitError(), 429),
        (SpotifyTimeoutError(), 504),
        (SpotifyAuthError(), 502),
    ],
)
async def test_search_maps_service_errors(session_factory, db_session, error, expected):
    user = await _seed_user(db_session)
    async with _build_client(session_factory, spotify=_FakeSpotify(error=error)) as client:
