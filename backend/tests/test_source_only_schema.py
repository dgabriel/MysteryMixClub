...
async def _seed_round(db_session) -> tuple[User, Mix]:
    user = User(email="s@example.com", display_name="S")
    db_session.add(user)
    await db_session.flush()
    league = Club(name="L", organizer_id=user.id, total_mixes=3, votes_per_player=3)
    db_session.add(league)
    await db_session.flush()
    db_session.add(ClubMember(club_id=league.id, user_id=user.id))
    round_ = Mix(club_id=league.id, mix_number=1, theme="t", state="open_submission")
