...
| `APPLE_PUSH_KEY_ID`, `APPLE_PUSH_PRIVATE_KEY` | SECRET | Push notifications (MysteryMixClub-4vii.25/27, IOS-04). Reuses `APPLE_MUSIC_TEAM_ID` above for the Team ID — not a separate value, and not gated on Apple Music being enabled. **Both or none** — either missing and every push send returns `"failed"` with no user-visible error, same "gap is a supported state" pattern. Provisioning walkthrough in `staging-setup.md` → "Enabling push notifications". |
