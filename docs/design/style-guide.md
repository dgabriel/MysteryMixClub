...
- **Rust is a signal.** Use it for the single most important piece of information or action on a screen — a status badge, a card accent line, a CTA arrow. Never use it twice in the same view.
  - **Exception — the nav brand mark.** The concentric-rings mark in the shared top nav may carry its single off-center Rust dot ("the fish") as persistent *brand identity*. Because the nav is global chrome, this dot does **not** count against a screen's one-Rust budget: a screen may still use Rust once in its own content. This is a standing exception to the one-use rule.
  - **Exception — Google's official Sign-In button (ADR 0007).** Google's branding terms require their button's logo, colors, and type to render unmodified — it cannot be reskinned into Sage/DM Mono. `GoogleSignInButton` (`frontend/src/components/GoogleSignInButton.tsx`) is the one component in the app that sits outside this system entirely, styled with Google's own brand values (as Tailwind arbitrary values, not named tokens, since those values aren't ours to name as design tokens). This is the second standing exception to the one-use rule.
  - **Exception — form validation errors (ADR 0004).** Invalid form fields are their own Rust category, separate from a screen's one decorative/informational use: every invalid field on a form may show Rust (underline + inline error text, with a small warning icon) at the same time, however many fields are currently invalid. A screen with a form does not also get to spend Rust decoratively — drop any decorative Rust (e.g. a `ConcentricRings` accent dot) on screens where form errors can appear. This category also covers a screen-level message that isn't tied to one field but is still about the user's own attempt — e.g. "you need an invite to create an account," an already-exists conflict, a rate limit — rendered the same way (Rust text + warning icon; see `FormError`, `frontend/src/components/FormError.tsx`). It does **not** extend to a message about an external system's outcome that isn't a claim anything the user did was invalid — e.g. a third-party sign-in being denied or cancelled — which stays plain Ink text. Do not extend Rust to a further category without stopping to ask first.
