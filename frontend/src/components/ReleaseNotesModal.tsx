 ...
 *
 * Uses the shared modal surface (ADR 0038): a white `paper` panel with a
 * dark `ink` border, `ink` copy, and `onPaper` controls.
 */
  ...
  return (
    <div className={MODAL_SCRIM}>
      <div
        ...
        className={`flex max-h-[80vh] w-full max-w-md flex-col ${MODAL_PANEL}`}
      >
        <div className="flex items-center justify-between gap-4 px-6 pt-5">
          ...
          <h2
            ...
            className="font-display text-[1.25rem] font-extrabold leading-[0.9] text-ink"
          >
            What&apos;s new
          </h2>
          <button type="button" onClick={onDismiss} aria-label="close" className={MODAL_CLOSE}>
            <CloseIcon />
          </button>
        </div>
        <div className="mt-4 flex-1 space-y-6 overflow-y-auto px-6">
          {RELEASE_NOTES.map((entry) => (
            <div key={entry.date}>
              <h3 className="font-mono uppercase tracking-mono-wide text-meta text-ink">
                {formatReleaseDate(entry.date)}
              </h3>
              <ul className="mt-2 list-disc space-y-1.5 pl-5">
                {entry.items.map((item) => (
                  <li key={item} className="text-sm leading-[1.72] text-ink">
                    {item}
                  </li>
                ))}
              </ul>
        ...
        <div className="px-6 pb-5 pt-6">
          <Button onPaper type="button" onClick={onDismiss} className="w-full">
            got it
          </Button>
        </div>
      </div>
